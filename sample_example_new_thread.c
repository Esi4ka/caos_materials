...
int main(){
	...
	//и передаем потоку указатель на принимаемый функцией аргумент  thread_data
	pthread_create(&thread, NULL, threadFunc, thread_data);
	
	//ждем завершения потока
	// pthread_join(thread, void** return_value); -- второй аргумент -- это возвращаемое значение
